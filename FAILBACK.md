...
- `silent` / `stall` / `network` (включая XHR `status=0` / CORS) считаются **вероятной цензурой**: хост НЕ уходит в quarantine и пере-пробуется по свежему соединению (до `silentRetriesPerHost` раз), плюс параллельно хеджируются другие кандидаты
